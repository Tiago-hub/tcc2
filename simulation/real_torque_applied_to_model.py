...
plt.title("Ângulo do quadril")
ax.set_ylabel("Ângulo (°)")
ax.plot(t, np.rad2deg(y[:,0]), 'b', label='Simulação')
ax.plot(t, (walk_data["steps"][0]["angle"]["hip"]), 'g', label='Referência')
ax.legend(loc='best')
#plt.xlim([0, 5])
#plt.ylim([-20, 20])
ax.grid()
...
ax.set_ylabel("Ângulo (°)")
ax.plot(t, np.rad2deg(y[:,2]), 'r', label='Simulação')
ax.plot(t, (walk_data["steps"][0]["angle"]["knee"]), 'y', label='Referência')
ax.legend(loc='best')
ax.grid()

ax = fig.add_subplot(3, 1, 3)
plt.title("Perfis de torques aplicados")
ax.plot(t, hip, 'b', label='Quadril')
# ax.plot(t,hip,'g', label='fuzzy hip')
#ax.plot(t, result[1], 'g', label='x2(t)')
ax.plot(t, knee, 'r', label='Joelho')
